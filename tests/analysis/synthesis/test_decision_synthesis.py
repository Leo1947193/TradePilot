...
def test_build_decision_synthesis_reads_event_report_flags_for_blocking_and_excluded_weights() -> None:
    decision = build_decision_synthesis(
        ModuleResults.model_validate(
            {
                "technical": {
                    "status": "usable",
                    "direction": "bullish",
                ...
                "event": {
                    ...
                    "direction": "bearish",
                    "data_completeness_pct": 100,
                    "summary": "Binary catalyst is close.",
                },
            }
        ),
        ModuleReports.model_validate(
            {
                "event": {
    ...
    assert decision.blocking_flags == ["binary_event_imminent"]
    assert decision.weight_scheme_used.available_weight_sum == 0.9
    assert decision.weight_scheme_used.available_weight_ratio == 0.9
    assert decision.module_contributions[1].status == ModuleStatus.EXCLUDED
